...
const Header = () => {
  ...
  const topIconVariants = {
    closed: { rotate: 0, y: 0 },
    open: { rotate: 45, y: 9, width: "100%" },
  };
  const middleIconVariants = {
      ...
      >
        <div className="container mx-auto flex justify-between items-center p-4 text-white">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <Link
              href="/doar"
              className="font-sans bg-[#4ECDC4] text-[#1A535C] font-bold py-2 px-4 rounded-full hover:bg-white transition-colors text-sm"
            >
              Doe Agora
            </Link>
          </div>
          ...
          <div className="md:hidden">
            <button
              ...
              className="z-50 relative w-7 h-6 flex flex-col justify-between"
            ...
            >
              ...
              />
              <motion.div
                className="w-10/12 h-0.5 bg-white rounded-full"
                variants={middleIconVariants}
                animate={isOpen ? "open" : "closed"}
                transition={{ duration: 0.2 }}
              />
              <motion.div
                className="w-8/12 h-0.5 bg-white rounded-full origin-center"
        ...
        {isOpen && (
          ...
          >
            <nav className="flex flex-col items-center space-y-8">
              {navLinks.map((link) => (
                <motion.div key={link.title} variants={navItemVariants}>
                  <Link
                    href={link.href}
                    className={`text-2xl font-sans font-bold text-white transition-colors ${
                      pathname === link.href
                        ? "text-[#4ECDC4]"
                        : "hover:text-[#4ECDC4]"
                    }`}
            ...
            >
              <Link
                href="/doar"
                className="font-sans bg-[#4ECDC4] text-[#1A535C] font-bold py-3 px-8 rounded-full hover:bg-white transition-colors text-lg"
                onClick={() => setIsOpen(false)}
              >
                Doe Agora
              </Link>
